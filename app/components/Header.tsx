  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => (
                <Link key={link.label} href={link.href} className={`px-3 py-2 text-sm font-medium rounded-lg transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800 hover:text-red-400' : 'text-gray-700 hover:bg-gray-50 hover:text-red-600'}`}>{link.label}</Link>
              ))}
            </nav>
              ...
              {loading ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <button onClick={() => setShowUserMenu(!showUserMenu)} className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800' : 'text-gray-700 hover:bg-gray-50'}`}>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={handleLoginClick} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800' : 'text-gray-700 hover:bg-gray-50'}`}>Sign In</button>
                  <button onClick={handleRegisterClick} className="px-5 py-2.5 text-sm font-semibold text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors shadow-sm">Sign Up</button>
                </div>
              )}
            </div>

            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className={`md:hidden p-2 rounded-lg ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`} aria-label="Toggle menu">
          ...
          {mobileMenuOpen && (
            <div className={`md:hidden border-t py-4 ${isDark ? 'border-gray-800' : 'border-gray-100'}`}>
              {navLinks.map((link) => (
                <Link key={link.label} href={link.href} className={`block px-4 py-3 text-base font-medium rounded-lg transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800 hover:text-red-400' : 'text-gray-700 hover:bg-gray-50 hover:text-red-600'}`} onClick={() => setMobileMenuOpen(false)}>{link.label}</Link>
              ))}
              <div className="mt-4 px-4 space-y-3">
                {user ? (
                  ...
                  <>
                    <button onClick={() => { setMobileMenuOpen(false); handleLoginClick() }} className={`block w-full text-center py-2.5 text-sm font-medium border rounded-lg ${isDark ? 'text-gray-300 border-gray-700' : 'text-gray-700 border-gray-200'}`}>Sign In</button>
                    <button onClick={() => { setMobileMenuOpen(false); handleRegisterClick() }} className="block w-full text-center py-2.5 text-sm font-semibold text-white bg-red-600 rounded-lg">Sign Up</button>
                  </>
                )}
              </div>
            </div>
      ...
      {!user && !loading && showBanner && (
        <div className="fixed bottom-4 left-0 right-0 sm:top-24 sm:bottom-auto z-40 pointer-events-none">
            ...
            <div className={"pointer-events-auto rounded-xl shadow-lg border transition-all duration-500 " + (isDark ? "bg-gray-800 border-gray-700 shadow-black/30" : "bg-white border-gray-200 shadow-lg")} style={{width: '28rem', maxWidth: '90vw'}}>
              <div className="px-4 py-3 flex items-start justify-between gap-3">
                <p className={"text-sm leading-relaxed " + (isDark ? "text-gray-300" : "text-gray-600")}>
                  You can start without signing up ÃÂ¢ÃÂÃÂ but the <button onClick={handleRegisterClick} className={"font-semibold underline underline-offset-2 " + (isDark ? "text-red-400 hover:text-red-300" : "text-red-600 hover:text-red-700")}>really cool features</button> unlock when you do.
                </p>
